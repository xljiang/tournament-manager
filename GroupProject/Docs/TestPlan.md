...
| S7.5        | Verify input for house cut, entry price are valid | JUnit test for valid input, Manual Test for error message layout | If not valid, show error message |checks that the housecut,entry price values are entered and that they are not null | pass|pass | v|if these fiels are null, appropriate error messages are displayed
